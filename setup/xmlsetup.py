...
def get_child_text(elem, tag_name, default=""):
    ...
    try:
        ...
        return default if text == "None" or text is None else text
    except:
        return default
...
def create_categories(categories):
    ...
    if categories is None:
        return
    logging.info(f'Found {categories.get("count")} categories')
...
def create_levels(levels):
    ...
    if levels is None:
        return
    logging.info(f'Found {levels.get("count")} game level(s)')
...
def create_hints(parent, box, flag=None):
    ...
    if parent and box:
        logging.info(f'Found {parent.get("count")} hint(s)')
...
def create_flags(parent, box):
    """ Create flag objects for a box """
    if not parent or not box:
        return
    logging.info(f'Found {parent.get("count")} flag(s)')
    flag_dependency = []
    for index, flag_elem in enumerate(parent):
        try:
            ...
            dbsession.add(flag)
            dbsession.flush()
            if depend := get_child_text(flag_elem, "depends_on", None):
...
def add_attachments(parent, flag):
    ...
    if flag is None:
        return
    logging.info(f'Found {parent.get("count")} attachment(s)')
...
def create_choices(parent, flag):
    ...
    if flag is None:
        return
    logging.info(f'Found {parent.get("count")} choice(s)')
...
def create_boxes(parent, corporation):
    ...
    if corporation is None:
        return
    logging.info(f'Found {parent.get("count")} boxes')
    for index, box_elem in enumerate(parent):
        try:
            ...
            if game_level is None:
                logging.warning(f"GameLevel does not exist for box {name}, skipping")
                ...
                if category := get_child_text(box_elem, "category"):
                ...
                logging.info(f"Box with name {name} already exists, skipping")
...
def create_corps(corps):
    ...
    if corps is None:
        return
    logging.info(f'Found {corps.get("count")} corporation(s)')
...
def update_configuration(config):
    ...
    copyfile(options.config, f"{options.config}.bak")
        ...
        try:
            if options[config_elem.tag] is not None:
                if config_elem.tag in images:
                    ...
                    lines = [line.text for line in get_child_by_tag(config, config_elem.tag)]
                    value = lines
                else:
                ...
                if isinstance(value, type(options[config_elem.tag])):
                    logging.info(f"Configuration ({config_elem.tag}): {value}")
                    options[config_elem.tag] = value
                else:
                    logging.error(
                        f"Confirguation ({config_elem.tag}): unable to convert type {type(value)} to {type(options[config_elem.tag])} for {value}"
                    )

        except BaseException as e:
            logging.exception(f"Failed to update configuration ({e})")
    save_config()
...
def _xml_file_import(filename):
    ...
    try:
        ...
        logging.debug(f"Done processing: {filename}")
        dbsession.commit()
        return True
    except:
        dbsession.rollback()
        logging.exception(
            f"Exception raised while parsing {filename}, rolling back changes"
        )
